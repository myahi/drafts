...
public class MarkitIncomingService {
    ...
    private volatile EmsJmsListener jms;

    public MarkitIncomingService() {
    }

    public void setJms(EmsJmsListener jms) {
        this.jms = jms;
    }
    ...
    public void handle(String messageContent) throws IOException, ErrorCode {
        ...
        try {
            ...
            if (Tools.MARKIT_ACTION_NEW.equals(markitAction)) {
                ...
                send(EmsJmsListener.OutgoingType.FUNCTIONAL, outgoing);
            ...
            if (isConnectionError(errorCode)) {
                send(EmsJmsListener.OutgoingType.TECHNICAL_ERROR, e.getMessage());
            } else {
                String outgoing = Tools.buildErrorToSource(inputMessageReference, String.valueOf(errorCode), e.toString());
                send(EmsJmsListener.OutgoingType.FUNCTIONAL_ERROR, outgoing);
            }
    ...
    private void send(EmsJmsListener.OutgoingType type, String payload) {
        EmsJmsListener local = this.jms;
        if (local == null) {
            throw new IllegalStateException("EmsJmsListener not set in MarkitIncomingService (call setJms(listener))");
        }
        local.send(type, payload);
    }
