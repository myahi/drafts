package fr.labanquepostale.marches.eai.core.io;
...
/**
 * Utility to list files with sorting + optional filename filter (Linux-like
 * glob), and store results into Camel Exchange properties: - inputFiles:
 * List<Path> - inputFileNames: List<String>
 */
public final class FileListingUtil {
	...
	public static List<Path> listFiles(Exchange exchange, Path dir, boolean recursive, SortCriterion sort, SortDirection direction, String filenamePattern) throws IOException {
		...
		if (filenamePattern != null && !filenamePattern.isBlank()) {
			try {
				...
				throw new IllegalArgumentException("Invalid filename glob pattern: '" + filenamePattern + "'. " + "Example: A_*.txt or Test_*_20260213.csv", ex);
			}
		}
		...
		try (Stream<Path> s = recursive ? Files.walk(dir) : Files.list(dir)) {
			...
			List<Path> files = s.filter(Files::isRegularFile).filter(p -> m == null || m.matches(p.getFileName())).sorted(cmp).collect(Collectors.toList());
