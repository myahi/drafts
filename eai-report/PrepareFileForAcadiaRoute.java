...
public class PrepareFileForAcadiaRoute extends RouteBuilder {
    ...
    public void configure() {
        ...
        fromF("file:{{eai.report.acadia.prepare.input.cso.dir}}"
                ...
                // listing des fichiers ICE via FileListingUtil
                ...
                .split(body()).stopOnException()
                    ...
                    .process(exchange -> {
                        ...
                        exchange.setProperty("skipCurrentFile", Boolean.FALSE);

                        // sécurité : si le CSO est dans le même répertoire et match le glob, on l'ignore
                        if (inputFileName.equals(csoFileName)) {
                            exchange.setProperty("skipCurrentFile", Boolean.TRUE);
                        }
                    ...
                    .filter(exchangeProperty("skipCurrentFile").isEqualTo(Boolean.FALSE))
                        ...
                        .process(exchange -> {
                            ...
                            exchange.getIn().setBody(OUTPUT_HEADER + "\n" + marshalledBody);
                        })
    ...
    private void handleException(Exchange exchange) throws Exception {
        ...
        if (exchange.getProperty("inputFilePath") != null) {
            FileMoveUtil.move(
                    exchange,
                    exchange.getProperty("errorsSubdir", String.class),
                    "inputFilePath"
            );
        }
