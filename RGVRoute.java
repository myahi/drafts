...
public class RGVRoute extends RouteBuilder {
	...
	private static final Logger LOGGER = LogManager.getLogger(RGVRoute.class);
	    ...
	    public void configure() throws Exception {
	        ...
	        .choice()
	            ...
	            .when(exchangeProperty("MTFormatSource").isEqualTo("MT014"))
	                .unmarshal().bindy(BindyType.Fixed, MT014.class).log("Processing record: ${body}").bean(MT014ToRGVMLConverter.class)
	            .when(exchangeProperty("MTFormatSource").isEqualTo("MT030"))
	                .unmarshal().bindy(BindyType.Fixed, MT030.class).log("Processing record: ${body}").bean(MT030ToRGVMLConverter.class)
	            .when(exchangeProperty("MTFormatSource").isEqualTo("MT031"))
	                .unmarshal().bindy(BindyType.Fixed, MT031.class).log("Processing record: ${body}").bean(MT031ToRGVMLConverter.class)
	            .when(exchangeProperty("MTFormatSource").isEqualTo("MT034"))
	                .unmarshal().bindy(BindyType.Fixed, MT034.class).log("Processing record: ${body}").bean(MT034ToRGVMLConverter.class)
	             .when(exchangeProperty("MTFormatSource").isEqualTo("MT035"))
	                .unmarshal().bindy(BindyType.Fixed, MT035.class).log("Processing record: ${body}").bean(MT035ToRGVMLConverter.class)
	             .when(exchangeProperty("MTFormatSource").isEqualTo("MT043"))
	                .unmarshal().bindy(BindyType.Fixed, MT043.class).log("Processing record: ${body}").bean(MT043ToRGVMLConverter.class)
	             .when(exchangeProperty("MTFormatSource").isEqualTo("MT060"))
	                .unmarshal().bindy(BindyType.Fixed, MT060.class).log("Processing record: ${body}")
	                .bean(TranscoBean.class)
	                .bean(MT060ToRGVMLConverter.class)
	             .when(exchangeProperty("MTFormatSource").isEqualTo("MT066"))
	                .unmarshal().bindy(BindyType.Fixed, MT066.class).log("Processing record: ${body}")
	                //.bean(TranscoBean.class)
	                .bean(MT066ToRGVMLConverter.class)
	             .when(exchangeProperty("MTFormatSource").isEqualTo("MT067"))
	                .unmarshal().bindy(BindyType.Fixed, MT067.class).log("Processing record: ${body}").bean(MT067ToRGVMLConverter.class)
	             .when(exchangeProperty("MTFormatSource").isEqualTo("MT090"))
	                .unmarshal().bindy(BindyType.Fixed, MT090.class).log("Processing record: ${body}").bean(MT090ToRGVMLConverter.class)
	             .when(exchangeProperty("MTFormatSource").isEqualTo("MT095"))
	                .unmarshal().bindy(BindyType.Fixed, MT095.class).log("Processing record: ${body}").bean(MT595ToRGVMLConverter.class)
	              .when(exchangeProperty("MTFormatSource").isEqualTo("MT595"))
	                .unmarshal().bindy(BindyType.Fixed, MT595.class).log("Processing record: ${body}").bean(MT060ToRGVMLConverter.class)
	   ...
	   private static void handleException(Exchange exchange, QName faultCode, String subCodeLocalName) {
		    ...
		    // SubCode custom (utile pour mapper côté client)
            //SoapFault fault = new SoapFault(msg, faultCode);
		    //fault.setSubCode(new QName("urn:errors", subCodeLocalName));
		    // Message d'erreur utilisateur (sans détails techniques)
		    // IMPORTANT : côté serveur SOAP, on met l'exception dans l'exchange
