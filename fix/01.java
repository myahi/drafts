...
import quickfix.fix50sp2.AllocationReportAck;   // adapte selon ton version/package
import quickfix.field.AllocReportID;            // 755
import quickfix.field.TransactTime;             // 60

import java.time.LocalDateTime;

private void sendAllocationReportAck(Message message, SessionID sessionID)
        throws FieldNotFound, SessionNotFound {

    AllocationReportAck ack = new AllocationReportAck();

    // 755 = AllocReportID (OBLIGATOIRE dans ton cas)
    ack.set(new AllocReportID(message.getString(AllocReportID.FIELD)));

    // 60 = TransactTime (souvent requis)
    ack.set(new TransactTime(LocalDateTime.now()));

    Session.sendToTarget(ack, sessionID);

    LOGGER.info("Allocation Report ACK sent for AllocReportID: {}",
            message.getString(AllocReportID.FIELD));
}
