...
# Normalisation: tout en minuscule (peu importe entrée user)
...
tmp="$(mktemp)"
awk -v pref="$PREFIX" '
  # lbp.ems.queueName=<prefix>.<reste>  => lbp.ems.queueName=pref.<reste>
  /^[[:space:]]*lbp\.ems\.queueName[[:space:]]*=/ {
    if (match($0, /^[[:space:]]*lbp\.ems\.queueName[[:space:]]*=[[:space:]]*[^.]*\.(.*)$/, m)) {
      $0 = "lbp.ems.queueName=" pref "." m[1]
    }
  }

  # lbp.ems.error.queueName=<prefix>.<reste> => lbp.ems.error.queueName=pref.<reste>
  /^[[:space:]]*lbp\.ems\.error\.queueName[[:space:]]*=/ {
    if (match($0, /^[[:space:]]*lbp\.ems\.error\.queueName[[:space:]]*=[[:space:]]*[^.]*\.(.*)$/, m)) {
      $0 = "lbp.ems.error.queueName=" pref "." m[1]
    }
  }
