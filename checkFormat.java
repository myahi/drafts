private boolean checkFormat(RGVmessage rgvMessage, Map<String, Transcos> transcosByCategory) {
    ...
    if (typeRemunerationLen == 0 &&
        findTransco(
                transcosByCategory,
                "rgv.mt065.typeRemuneration.filter",
    ...
    if ((programOriginLen == 0 &&
         findTransco(
                 transcosByCategory,
                 "rgv.mt065.programOrigin.filter",
                 Map.of("srcApp", "Calypso", "srcVl", "YES")
         ) != null)
        || programOriginLen > 3) {
        return true;
