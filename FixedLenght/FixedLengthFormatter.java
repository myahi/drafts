 ...
 * - Supports per-field and global (default) padding/alignment.
 */
@Component
public class FixedLengthFormatter {
    ...
    public static class Options {
        ...
        // ✅ Global defaults for padding/alignment (used only if not set at field level)
        ...
        public Align getDefaultAlign() { return defaultAlign; }
        public char getDefaultPaddingChar() { return defaultPaddingChar; }
        ...
        public Options defaultAlign(Align a) { this.defaultAlign = a; return this; }
        public Options defaultPaddingChar(char c) { this.defaultPaddingChar = c; return this; }

        public static Options defaults() { return new Options(); }
    }

    /**
     * Field definition (Bindy-like capabilities, but generic).
     * Values are taken from Map by field "name".
     */
    public static class FieldDef {
        private final String name;
        private final int length;

        // ✅ Nullable => inherit from Options if not set
        private Align align = null;
        private Character paddingChar = null;
        ...
        // Formatting options (pattern/precision/impliedDecimal)
        ...
        // Inheritance-aware getters
        public Align getAlignOrDefault(Align fallback) { return align != null ? align : fallback; }
        public char getPaddingCharOrDefault(char fallback) { return paddingChar != null ? paddingChar : fallback; }

        // Compatibility getters (not used by formatter internals)
        public Align getAlign() { return align != null ? align : Align.LEFT; }
        public char getPaddingChar() { return paddingChar != null ? paddingChar : ' '; }
    ...
    public String formatLine(List<FieldDef> layout, Map<String, ?> values, Options options) {
        ...
        int totalLen = layout.stream().mapToInt(FieldDef::getLength).sum();
        ...
        for (FieldDef f : layout) {
            ...
            sb.append(padAndClip(s, f, options));
        }
        return sb.toString();
    }
    ...
    private String renderValue(Object raw,
        ...
        if (!keyPresent) {
            ...
            return sanitize(f.getDefaultValue(), options);
        }
        ...
        if (raw == null) {
            ...
            return sanitize(f.getNullValue(), options);
        }

        if (f.getFormatter() != null) {
            return sanitize(f.getFormatter().apply(raw), options);
        }
        ...
        if (s.isEmpty() && !f.isKeepEmptyAsEmpty()) {
            s = sanitize(f.getDefaultValue(), options);
        }
    ...
    private String padAndClip(String s, FieldDef f, Options options) {
        if (s == null) s = "";
        ...
        Align align = f.getAlignOrDefault(options.getDefaultAlign());
        char padChar = f.getPaddingCharOrDefault(options.getDefaultPaddingChar());

        StringBuilder out = new StringBuilder(f.getLength());
        if (align == Align.RIGHT) {
            for (int i = 0; i < pad; i++) out.append(padChar);
            out.append(s);
        } else {
            out.append(s);
            for (int i = 0; i < pad; i++) out.append(padChar);
